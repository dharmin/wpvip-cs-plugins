<?php
/**
 * Wistia Anti Mangler(Legacy).
 * 
 * @package wistia-wordpress-oembed-plugin
 */

/**
 * Wistia Anti Mangler Class(Legacy).
 */
class WistiaAntiMangler {

	/**
	 * Unfiltered Substrings.
	 * 
	 * @var array
	 */
	 ...
	 *
	 * @param  regexp $needle String to find.
	 * @param  string $haystack String from find the given string.
	 *
	...
	function all_tag_ranges_for_match( $needle, $haystack ) {
		$result = array();
		$offset = 0;
		$range  = $this->whole_tag_range_for_match( $needle, $haystack, $offset );
		while ( $range ) {
	 ...
	 * 
	 * @param int    $range range or offset and length for substr.
	 * @param string $haystack String from find the given string.
	 */
	 ...
	 * 
	 * @param array  $ranges range or offset and length for substr.
	 * @param string $haystack String from find the given string.
	 */
	 ...
	 * 
	 * @param array  $range1 range or offset for substr.
	 * @param array  $range2 range or length for substr.
	 * @param string $haystack String from find the given string.
	 */
	 ...
	 * 
	 * @param  string $haystack String from find the tag.
	...
	function all_ranges( $haystack ) {
		return array_merge(
			$this->all_iframe_ranges( $haystack ),
	 ...
	 * 
	 * @param  string $text String from find the tag.
	 * @return array Array of result.
	 */
	function find_existing_scripts( $text ) {
		...
		foreach ( $script_ranges as $range ) {
			...
			$matches     = preg_match( '/src=["\'](?:https?)?\/\/fast\.wistia\.com\/static\/concat\/(.*?)\.js/', $script_text );
			if ( ! empty( $matches ) ) {
	 ...
	 * @param  int    $offset  Optional, the offset to start searching from.
	 *
	 * @return  string Substring from text and range
	 */
	...
	function infer_required_scripts( $text ) {
		...
		foreach ( $script_ranges as $range ) {
			...
			if ( ! preg_match( '/src=["\']/', $script_text ) ) {
				$func_text = $this->find_whole_function_call( 'Wistia.embed', $script_text );
				if ( ! empty( $func_text ) ) {
				...
				$func_text = $this->find_whole_function_call( 'Wistia.playlist', $script_text );
				if ( ! empty( $func_text ) ) {
					if ( preg_match( '/["\']?version["\']?:\s*["\'](v\d+)["\']/', $func_text, $match ) ) {
						$version = $match[1];
				...
				$func_text = $this->find_whole_function_call( 'Wistia.plugin.socialbar', $script_text );
				if ( $func_text ) {
					if ( preg_match( '/["\']?version["\']?:\s*["\'](v\d+)["\']/', $func_text, $match ) ) {
						array_push( $required_scripts[0], 'socialbar-' . $match[1] );
					}
				}
				$func_text = $this->find_whole_function_call( 'Wistia.plugin.postRoll', $script_text );
				if ( $func_text ) {
					if ( preg_match( '/["\']?version["\']?:\s*["\'](v\d+)["\']/', $func_text, $match ) ) {
						array_push( $required_scripts[0], 'postRoll-' . $match[1] );
					}
				}
				$func_text = $this->find_whole_function_call( 'Wistia.plugin.transcript', $script_text );
				if ( $func_text ) {
					if ( preg_match( '/["\']?version["\']?:\s*["\'](v\d+)["\']/', $func_text, $match ) ) {
						array_push( $required_scripts[0], 'transcript-' . $match[1] );
					}
				}
				$func_text = $this->find_whole_function_call( 'Wistia.plugin.requireEmail', $script_text );
				if ( $func_text ) {
					if ( preg_match( '/["\']?version["\']?:\s*["\'](v\d+)["\']/', $func_text, $match ) ) {
	 ...
	 * them.
	 * 
	 * @param string $scripts  Name of scripts to enqueue.
	 */
	function concat_script_tag( $scripts ) {
		$script_name = implode( $scripts, '%2C' );
		$script      = "https://fast.wistia.com/static/concat/$script_name.js";
		add_action(
			'wp_enqueue_scripts',
			function() {
				wp_enqueue_script( $script_name, $script, false );
			} 
		);
